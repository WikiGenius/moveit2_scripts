#ifndef DRAW_X_NODE_HPP
#define DRAW_X_NODE_HPP
...
public:
   ...
   * handling)
   */
  DrawXtNode(const rclcpp::NodeOptions &options);
...
private:
  ...
  const double DELTA = 0.05; ///< Distance DELTA to move X (in meters)
  ...
  // Logging setup
  const rclcpp::Logger LOGGER = rclcpp::get_logger("draw_x");
  // -------------------------- Private Methods ----------------------------
   ...
   */
  bool draw_X(const geometry_msgs::msg::Pose &pre_grasp_pose,
              const MotionParams &params);
};
#endif // DRAW_X_NODE_HPP
